...
class InferenceHelper:
    def __init__(self, dataset='kitti', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            pretrained_path = "/ssd/ll/AdaBins/checkpoints_5/UnetAdaptiveBins_27-Aug_15-45-nodebs16-tep300-lr0.000357-wd0.1-d29742a3-cb17-40a8-bd55-b1d7e6daa1a1_latest.pt"
        elif dataset == 'kitti':
            self.min_depth = 1e-3
            self.max_depth = 80
    ...
    def predict_dir(self, test_dir, out_dir):
        ...
        transform = ToTensor()
        # /data1/dataset/VIPER/test/img/047/047_00805.jpg
        _test_dir = test_dir
        test_dir = "/"+_test_dir.split("/")[1]+"/"+_test_dir.split("/")[2]+"/"+_test_dir.split("/")[3] + "/"+_test_dir.split("/")[4]+"/"+_test_dir.split("/")[5]+"/"+_test_dir.split("/")[6]
        # print(test_dir)
...
if __name__ == '__main__':
    ...
    img_names = []
    with open('train_test_inputs/viper_test_files.txt', 'r') as f:
        for line in f.readlines():
            img_names.append(line.split(".")[0] + '.jpg')
    num_images = len(img_names)

    inferHelper = InferenceHelper()
    for i in range(num_images):
        inferHelper.predict_dir(img_names[i], "predictions_viper")
