    ...
    with torch.no_grad():
        ...
        for batch in tqdm(sequential):
            ...
            if args.save_dir is not None:
                ...
                pred_path = os.path.join(args.save_dir + "/" + dpath[0] + "/" + dpath[1], f"{dpath[-1].split('.')[0]}.png")
                
                os.makedirs(os.path.dirname(pred_path), exist_ok=True)
                pred = (final * factor).astype('uint16')
                Image.fromarray(pred).save(pred_path)
                # print(Image.fromarray(pred).mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--do_kb_crop', help='Use kitti benchmark cropping', action='store_false')
